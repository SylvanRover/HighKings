...
public partial class TouchKit
{
	...
	private float _scrollWheelTotal = 0f;
	private float scrollScale = 20f;
	private Vector3 startingHalfTouchOffset = new Vector3(200f, 0f,0f);
	/// <summary>
	...
	private bool shouldProcessMouseInput()
	{
		...
		if( simulateMultitouch && ( _hasActiveSimulatedMultitouch || Input.GetKey( KeyCode.LeftAlt ) || Input.GetKeyUp( KeyCode.LeftAlt ) || simulatePinchWithScrollwheel ) )
		{
			float delta = Input.GetAxis("Mouse ScrollWheel");
			TouchPhase? touchPhase = null;

			if (simulatePinchWithScrollwheel){
				if (delta>0f || delta<0f){
					...
					if( !_hasActiveSimulatedMultitouch ){
						...
						_simulatedMultitouchStartPosition = Input.mousePosition + startingHalfTouchOffset;
						_simulatedMousePosition= Input.mousePosition - startingHalfTouchOffset;
						_scrollWheelTotal = delta;
					}
					else
					{
						touchPhase = TouchPhase.Moved;
						_scrollWheelTotal+=delta * scrollScale;
					}
					if (_scrollWheelTotal<0 && _scrollWheelTotal<=-startingHalfTouchOffset.x){
						touchPhase = TouchPhase.Ended;
						_hasActiveSimulatedMultitouch = false;
						_scrollWheelTotal = 0f;

					}else{
						Vector3 diff = new Vector3( _scrollWheelTotal, 0f, 0f);
						_simulatedMultitouchStartPosition = Input.mousePosition + startingHalfTouchOffset + diff;
						_simulatedMousePosition = Input.mousePosition - startingHalfTouchOffset - diff;
					}
				}else if (_hasActiveSimulatedMultitouch){
					touchPhase = TouchPhase.Ended;
					_hasActiveSimulatedMultitouch = false;
					_scrollWheelTotal = 0f;
				}
			...
			{
				if (simulatePinchWithScrollwheel){
					_liveTouches.Add( _touchCache[0].populateWithPosition( _simulatedMultitouchStartPosition.Value, touchPhase.Value ) );
				}
		...
		_hasActiveSimulatedTouch = Input.GetMouseButton( 0 ) || _scrollWheelTotal!=0f;
