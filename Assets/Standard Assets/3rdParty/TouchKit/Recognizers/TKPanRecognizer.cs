...
public class TKPanRecognizer : TKAbstractGestureRecognizer
{
	...
	public TKPanRecognizer( float minPanDistance = 0)
	{
	...
	internal override bool touchesBegan( List<TKTouch> touches )
	{
		...
		if( state == TKGestureRecognizerState.Possible || ( ( state == TKGestureRecognizerState.Began || state == TKGestureRecognizerState.RecognizedAndStillRecognizing ) ) )
		{
			if(touches.Count > maximumNumberOfTouches || _trackingTouches.Count >= maximumNumberOfTouches) {
	...
	{
		if (touches.Count > maximumNumberOfTouches) {
			state = TKGestureRecognizerState.FailedOrEnded;
		} else {
