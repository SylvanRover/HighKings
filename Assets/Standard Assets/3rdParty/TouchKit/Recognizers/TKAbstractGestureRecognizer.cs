...
public abstract class TKAbstractGestureRecognizer : IComparable<TKAbstractGestureRecognizer>
{
	...
	internal void recognizeTouches( List<TKTouch> touches )
	{
		...
		for( var i = touches.Count - 1; i >= 0; i-- )
		{
			...
			switch( touch.phase )
			{
				...
				if( !_sentTouchesBegan && isTouchWithinBoundaryFrame( touches[i] ) && meetsValidation( touches[i] ))
					{
	...
	public System.Func<TKTouch, bool> customTouchValidator;

	internal bool meetsValidation (TKTouch touch)
	{
		if (customTouchValidator != null) {
			Delegate[] invocationList = customTouchValidator.GetInvocationList ();
			for (int i = 0; i < invocationList.Length; ++i) {
				if (!((System.Func<TKTouch, bool>)invocationList [i]) (touch)) {
					return false;
				}
			}
		}
