    ...
    public class VectorLine {
        ...
        public Joins joins {
            get {
                ...
                if(!m_continuous && value == Joins.Fill) {
                    return;
                }
                m_joins = value;
            }
        ...
        //Digit addition
        public int GetSegmentIndexNearest01(float percentDistance) {

            if(m_distances == null) {
                SetDistances();
            }

            float distanceLength = Mathf.Lerp(0.0f, GetLength(), percentDistance);

            //Binary search
            int low = 0, high = m_distances.Length - 1, midpoint = 0;
            while(low < high) {
                midpoint = low + (high - low) / 2;
                if(distanceLength < m_distances[midpoint]) {
                    high = midpoint - 1;
                } else {
                    low = midpoint + 1;
                }
